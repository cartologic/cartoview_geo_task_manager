 ...
 */
angular.module('cartoview.viewer.editor').directive('taskManagerConfig',  function(urlsHelper) {
    var fieldTypes = [{
        label: "Text",
        name: "text",
    ...
    return {
        ...
        controller: function ($scope, dataService, $tastypieResource, $mdDialog) {
            ...
            $scope.config = $scope.instanceObj.config.taskManager = $scope.instanceObj.config.taskManager || {};
            ...
            dataService.onMapSelect(function () {
                $scope.config = $scope.instanceObj.config.taskManager = {};
                populateLayers();
            });
            var initAttributes = function () {
                attributesResource.objects.$find({layer__typename: $scope.config.layer}).then(function () {
                    var attributes = [];
                    angular.forEach(attributesResource.page.objects, function (a) {
                        if (a.attribute_type.indexOf("gml") == 0) return;
            ...
            $scope.showAttributeSettings = function (ev, attribute) {
                $mdDialog.show({
                    ...
                    templateUrl: urlsHelper.static + 'task_manager/angular-templates/attribute-settings-dialog.html?' + new Date().getTime(),

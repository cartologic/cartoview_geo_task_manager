 ...
 */
var taskManagerApp = angular.module('cartoview.taskManagerApp', ['cartoview.mapViewerApp', 'ngResourceTastypie', 'cartoview.userEngage']);
taskManagerApp.config(function($tastypieProvider, $httpProvider, urlsHelper){
    ...
    $tastypieProvider.add('cartoview', {url: urlsHelper.rest + "../rest/"});
    ...
    var save = function () {
        ...
        service.task.$save().then(function(result) {
            ...
            $http({
                ...
                service.busy = false;
                getWMSLayer(config.layer).get('source').updateParams({"time": Date.now()});
            });
        },
        function(error) {
            console.log(error);
